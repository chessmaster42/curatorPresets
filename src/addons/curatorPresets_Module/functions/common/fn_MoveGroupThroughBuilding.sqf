...
if (_buildingscount < 1) exitWith {
	"Error - No buildings within area" call bis_fnc_logFormat;
};
...
if(_buildingPosCount < 1) exitWith {
	"Error - No searchable buildings found" call bis_fnc_logFormat;
};

format["Moving %1 through building %2 with %3 positions", _group, _building, _buildingPosCount] call bis_fnc_logFormat;
